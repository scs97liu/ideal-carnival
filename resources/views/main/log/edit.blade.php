...
@section('content')
    <div class="portlet box">
        <div class="portlet-body form">
            <form role="form" action="{{ route('log.update', $log->id) }}" method="POST">
                {{ method_field('PUT') }}
                <div class="form-body">
                    <h4 class="block">Date/Time</h4>
                    <div class="form-group">
                        <div class="input-group input-group-lg input-xlarge date form_datetime">
                            <input type="text" size="16" name="datetime" readonly class="form-control" value="{{ $log->time->format('d F Y - H:i') }}">
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                <div class="input-group input-group-lg">
                                    <input type="number" min="0" step="0.1" name="bg" class="form-control" value="{{ $log->bg->bg or null }}">
                    ...
                    <div class="row">
                        <div class="col-sm-2">
                            <div class="form-group">
                                <div class="input-group input-group-lg">
                                    <input type="number" min="0" step="0.5" name="carbs" class="form-control" value="{{ $log->carb->carbs or null }}">
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                <select class="form-control input-lg" name="insulin-types[]">
                                    <option {{ ($medication->type === "Fast") ? 'selected' : null }}>Fast</option>
                                    <option {{ ($medication->type === "Slow") ? 'selected' : null }}>Slow</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-1">
                    ...
                    @endforeach
                    @if(count($log->medications) == 0)
                        <div class="row">
                            <div class="col-sm-2">
                                <div class="form-group">
                                    <div class="input-group input-group-lg">
                                        <input type="number" min="0" step="1" name="insulin[]" class="form-control">
                                        <span class="input-group-addon" id="sizing-addon1">U</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-sm-2">
                            <div class="form-group">
                                <div class="input-group input-group-lg">
                                    <input type="number" min="0" step="1" name="exercise" class="form-control" value="{{ $log->exercise->minutes or null }}" >
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                <select class="form-control input-lg" name="exercise-intensity">
                                    @if($log->exercise)
                                        <option {{ ($log->exercise->difficulty === 'Hard') ? 'Hard' : null }}>Hard</option>
                                        <option {{ ($log->exercise->difficulty === 'Medium') ? 'Medium' : null }}>Medium</option>
                                        <option {{ ($log->exercise->difficulty === 'Easy') ? 'Easy' : null }}>Easy</option>
                                    @else
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                                <textarea class="form-control" name="additional-notes" style="width:100%" rows="10">{{ $log->notes->first()->text or null }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="form-actions left">
                    ...
                    <button type="submit" class="btn btn-lg green">Update</button>
                </div>
                @include('main.log._editmodal')
            </form>
        </div>
    </div>
@endsection
